...
var Main = function () {
    ...
    var runNavigationToggler = function () {
        ...
        $('.clip-chevron-right-views').css({'display':'none'});
        $('.navigation-toggler').on('click', function () {

            if (!$('body').hasClass('navigation-small')) {
                $('body').addClass('navigation-small');
                $('.clip-chevron-left-views').css({'display':'none'});
                $('.clip-chevron-right-views').css({'display':'inline-block'});
            } else {
                $('body').removeClass('navigation-small');
                $('.clip-chevron-right-views').css({'display':'none'});
                $('.clip-chevron-left-views').css({'display':'inline-block'});
            }
    ...
    var runSearchInput = function () {
        ...
        search_button.on('click', function () {
            if ($(search_input).hasClass('open') && $(search_input).val() == '') {
    ...
    var setColorScheme = function () {
        $('.icons-color a').on('click', function () {
            ...
            $('#skin_color').attr("href", "assets/css/theme1/" + $(this).attr('id') + ".min.css");
    ...
    var setLayoutStyle = function () {
        $('select[name="layouts"]').change(function () {
            if ($('select[name="layouts"] option:selected').val() == 'boxed')
    ...
    var runCustomSetting = function () {
        if ($.cookie("clip-setting")) {
            ...
            if (loadSetting.layoutBoxed) {
                $('body').addClass('layout-boxed');
                $('#style_selector select[name="layouts"]').find('option[value="boxed"]').attr('selected', 'true');
            };
    ...
    var runDefaultSetting = function () {
        $('#style_selector select[name="layouts"]').val('default');
    ...
    //function to set layouts from menu
    var runLayoutMenu = function () {
        $(".set-boxed-layouts").on("click", function () {
            $('body').addClass('layout-boxed');
            return false;
        });
    ...
    // function to dropdown-menu-child appear
    var runMenuAppear = function () {
        var item=$(".dropdown-menu-item");
        item.mouseover(function () {
            if(item.children.length > 0){
                var top=100/(item.length)*$(this).index()+"%";
                $(this).css({"background":'#C83A2A',"color":'white'});
                $(this).find("ul",'dropdown-menu-child').css({"display": 'inline-block',"top":top});
            }
        });
    ...
    var runIcon=function () {
        $("btn-icon").append("<i class='fa fa-arrow-circle-o-right'></i>");
    };
