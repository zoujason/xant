...
var Main = function () {
    ...
    function runIcon() {
        $("btn-icon").append("<i></i>");
        $("btn-icon").children().addClass("fa");
        $("btn-icon").children().addClass("fa-arrow-circle-right");
    }

    //function to tab-content-inside can bo open
    var runTab = function (v) {
        var tab_pane = document.getElementsByClassName("tab-pane");
        for(var i = 0; i < tab_pane.length; i++) {

            var tab_panes = tab_pane[i];

            if(tab_panes == document.getElementById("panel_tab4_example" + v )) {
                tab_panes.style.display = "block";
            } else {
                tab_panes.style.display = "none";
            }
        }
    };
